...
| [Validation & Preflight](docs/README_VALIDATION.md) | All 18 preflight checks, what fails and how to fix it |
...
- 18 preflight checks: storage engine, primary key shape, invisible columns, FK indexes, FK coverage (external + internal), destination schema compatibility, source/destination/tracking-schema permissions, DELETE triggers, destination INSERT triggers, CASCADE warnings
...
- **MySQL native partitioning** — if your table is partitioned by date and you do not need to keep the old rows, `ALTER TABLE … DROP PARTITION` is dramatically faster than any archiving tool. Use that instead.
