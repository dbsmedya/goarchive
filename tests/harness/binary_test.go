...
func runnerScript(t *testing.T, explicit, goExit string) (string, []string, string) {
	...
	goScript := "#!/usr/bin/env bash\necho build >> \"$BUILD_LOG\"\nif [[ \"$1\" == build && \"$2\" == -o && \"$3\" != \"\" ]]; then printf current > \"$3\"; fi\nexit " + goExit + "\n"
...
func TestHarnessBuildFailure(t *testing.T) {
	...
	copyRunner(t, root)
	if err := runEnsure(t, root, env); err == nil { t.Fatal("HARNESS_FAILED_BUILD_REUSED: failed build succeeded") }
	data, err := os.ReadFile(bin); if err != nil { t.Fatal(err) }
	if string(data) != "current" { t.Fatalf("failed builder did not run: %q", data) }
	if err := runEnsure(t, root, env); err == nil { t.Fatal("second failed build succeeded") }
	lines, _ := os.ReadFile(filepath.Join(root, "build.log"))
